...
async function fetchVideosByChannel(channelId, maxResults = 5) {
    try {
        ...
        const playlistItemsRes = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&playlistId=${uploadsPlaylistId}&maxResults=${maxResults}&key=${API_KEY}`);
        ...
        const durationsRes = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${videoIds}&key=${API_KEY}`);
...
export default function YouTubePlaylistPlayer() {
  ...
  useEffect(() => {
    if (!userId || !initialVideoLoaded.current) return;
      ...
      const unsubscribe = onSnapshot(videoMetaRef, (snap) => {
        if (snap.exists()) {
          ...
          if (!meta.author || !meta.viewCount || !meta.channelId) {
            fetch(`https://www.googleapis.com/youtube/v3/videos?part=snippet,statistics&id=${currentVideoId}&key=${API_KEY}`)
              .then(res => res.json())
              .then(data => {
  ...
  const handleAuthorClick = async () => {
    ...
    const videos = await fetchVideosByChannel(videoChannelId, 5);
  ...
  const handleSearch = async (e) => {
    ...
    try {
      const res = await fetch(`https://www.googleapis.com/youtube/v3/search?part=snippet&type=video&maxResults=5&q=${encodeURIComponent(searchQuery)}&key=${API_KEY}`);
      const data = await res.json();
      if (data.items) {
        const videos = data.items.map(item => ({ id: item.id.videoId, title: item.snippet.title, duration: 0 }));
        const videoIds = videos.map(v => v.id).join(',');
        const durRes = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails&id=${videoIds}&key=${API_KEY}`);
  ...
  useEffect(() => {
    ...
    processPlaylists();
  }, [playlists, userId, currentPlaylistIndex, chronologicalFilter]);
  ...
  const fetchAllVideos = async (playlistId, playlistIndex) => {
    ...
    if (isFetchingAnyPlaylist.current || fetchingPlaylists.current.has(playlistId)) {
      console.log('🚫 Skipping fetch for', playlistId, '- already in progress');
      return;
    }
    ...
    try {
      ...
      do {
        const res = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet,contentDetails&maxResults=50&playlistId=${playlistId}&key=${API_KEY}${nextPageToken ? `&pageToken=${nextPageToken}` : ""}`);
        const data = await res.json();
        if (data.items) {
      ...
      if (idsToFetch.length > 0) {
        for (let i = 0; i < idsToFetch.length; i += 50) {
          ...
          const durRes = await fetch(`https://www.googleapis.com/youtube/v3/videos?part=contentDetails,snippet,statistics&id=${batchIds}&key=${API_KEY}`);
        const durData = await durRes.json();
  ...
  const handleAddPlaylist = async () => {
    ...
    try {
      const res = await fetch(`https://www.googleapis.com/youtube/v3/playlists?part=snippet&id=${newPlaylistId}&key=${API_KEY}`);
      const data = await res.json();
